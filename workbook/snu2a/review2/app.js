(function(){
  ...
  const STORAGE_KEY = 'snu2a_workbook_review2_eval_v1';
  ...
  function makeAttemptId(){
    return `wb2a_r1_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,9)}`;
  }
  ...
  function widenAnswerBlanks(html, q){
    const rawAnswer = q && Array.isArray(q.options) && Number.isInteger(q.answer)
    ...
    return String(html).replace(/\(\s{2,}\)/g,
      `<span class="answer-blank">(<span class="blank-measure" aria-hidden="true">${safeMeasure}</span>)</span>`);
  }
